...
def train_model_retina_net(model = None, dataset = None,  optimizer = None, 
	expt = "", it = 0, batch_size = 1, n_epoch = 100,
	direc_save = "/home/vanvalen/ImageAnalysis/DeepCell2/trained_networks/", 
	direc_data = "/home/vanvalen/ImageAnalysis/DeepCell2/training_data_npz/", 
	lr_sched = rate_scheduler(lr = 0.01, decay = 0.95),
	...
	print('Testing data shape:', val_dict["channels"].shape)
	print('Testing labels shape:', val_dict["channels"].shape)
	...
	# fit the model on the batches generated by datagen.flow()
	loss_history = model.fit_generator(datagen.flow(train_dict, batch_size = batch_size, return_image = False),
						steps_per_epoch = train_dict["channels"].shape[0]/batch_size,
						epochs = n_epoch,
						validation_data = datagen.flow(val_dict, batch_size = batch_size, return_image = False),
						validation_steps = val_dict["channels"].shape[0]/batch_size,
						callbacks = [ModelCheckpoint(file_name_save, monitor = 'val_loss', verbose = 1, save_best_only = True, mode = 'auto'),
							LearningRateScheduler(lr_sched)])
